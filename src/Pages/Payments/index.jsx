...
const Payments = () => {
  ...
  const handleMarkAsPaid = async (user) => {
    ...
    try {
      await axios.post(`${API_BASE}/admin/payout-confirm`, 
    ...
    <div className="space-y-6 p-6 bg-[#090909] min-h-screen text-white">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400">Manage Transactions & Affiliate Payouts</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <StatCard title="Total Revenue" value={formatCurrency(mockStats.totalRevenue)} icon={DollarSign} color="text-green-500"/>
        {/* Dynamic Pending Payout Calculation */}
        <StatCard 
            title="Pending Payouts" 
            value={formatCurrency(payouts.reduce((sum, p) => sum + p.walletBalance, 0))} 
            icon={Wallet} 
            color="text-red-400" 
        />
        <StatCard title="Monthly Revenue" value={formatCurrency(mockStats.monthlyRevenue)} icon={TrendingUp} color="text-blue-400"/>
        <StatCard title="Success Rate" value={`${mockStats.successRate}%`} icon={CheckCircle} color="text-orange-500"/>
      </div>
      ...
      <Tabs defaultValue="transactions" className="w-full" onValueChange={setActiveTab}>
        <TabsList className="bg-[#1a1818] border border-[#2a2828] p-1">
          <TabsTrigger value="transactions" className="data-[state=active]:bg-[#2a2828]">Transactions</TabsTrigger>
                    ...
                    payouts.map((user) => (
                      <TableRow key={user._id} className="border-[#2a2828] hover:bg-[#1f1d1d]">
                        ...
                        <TableCell className="text-right">
                            ...
                            >
                                {processingId === user._id ? <Loader2 className="w-3 h-3 animate-spin"/> : "Mark Paid"}
                            </Button>
                        </TableCell>
                      </TableRow>
                    ))
          ...
          {selectedPayment && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div><p className="text-sm text-gray-400">Transaction ID</p><p className="text-white font-mono">{selectedPayment.id}</p></div>
                ...
                <div><p className="text-sm text-gray-400">User</p><p className="text-white">{selectedPayment.user}</p></div>
                <div><p className="text-sm text-gray-400">Email</p><p className="text-white">{selectedPayment.email}</p></div>
                ...
                <div><p className="text-sm text-gray-400">Plan</p><p className="text-white">{selectedPayment.plan}</p></div>
                <div><p className="text-sm text-gray-400">Method</p><p className="text-white capitalize">{selectedPayment.method}</p></div>
